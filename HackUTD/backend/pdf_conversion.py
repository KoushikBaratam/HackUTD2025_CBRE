...
# Scopes for Google Docs and Drive access
SCOPES = [
    "https://www.googleapis.com/auth/documents",
    "https://www.googleapis.com/auth/drive.file"
]

def export_summaries_to_google_doc_oauth(doc_title="ClauseChain_Summary_Report"):
    """Create a Google Doc containing summaries using local OAuth login."""
    print("🔐 Authenticating with your Google account...")

    creds = None
    token_path = "token.json"

    # Reuse existing credentials if token exists
    ...
    JSON_PATH = "pdf_summaries.json"
    if not os.path.exists(JSON_PATH):
        print("⚠️ No summaries found to export.")
        return None

    with open(JSON_PATH, "r") as f:
        data = json.load(f)
    entries = data.get("pdf_summaries", [])
    ...
    print(f"✅ Exported {len(entries)} summaries to Google Docs.")
