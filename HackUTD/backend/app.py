...
@app.route('/api/export_docs', methods=['POST'])
def export_docs():
    try:
        doc_link = pdf_conversion.export_summaries_to_google_doc_oauth()
        if not doc_link:
            return jsonify({"success": False, "message": "No summaries found"}), 400
        return jsonify({
            "success": True,
            "message": "Summaries exported successfully to Google Docs",
            "doc_link": doc_link
        }), 200
    except Exception as e:
        print("❌ Error exporting to Google Docs:", e)
