                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center space-x-2 ml-4">
                      ...
                      <button
                        onClick={async () => {
                          try {
                            const response = await fetch('http://localhost:5000/api/export_docs', {
                              method: 'POST',
                              headers: { 'Content-Type': 'application/json' },
                            });
                            const data = await response.json();
                            if (data.success && data.doc_link) {
                            ...
                            alert('❌ Error exporting to Google Docs. Check console.');
                          }
                        }}
                        className="px-3 py-1.5 text-sm text-gray-600 hover:bg-gray-50 rounded-md border border-gray-300 transition-colors"
                      >
                        Download
                      </button>
